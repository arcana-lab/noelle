...
static llvm::RegisterPass<WPAPass> WHOLEPROGRAMPA(
    "wpa",
...
static llvm::cl::bits<PointerAnalysis::PTATY> PASelected(
    ...
    llvm::cl::values(
        clEnumValN(PointerAnalysis::Andersen_WPA,
                   "nander",
                   "Standard inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenLCD_WPA,
                   "lander",
                   "Lazy cycle detection inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenHCD_WPA,
                   "hander",
                   "Hybrid cycle detection inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenHLCD_WPA,
                   "hlander",
                   "Hybrid lazy cycle detection inclusion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenSCD_WPA,
                   "sander",
                   "Selective cycle detection inclusion-based analysis"),
        clEnumValN(
            PointerAnalysis::AndersenSFR_WPA,
            "sfrander",
            "Stride-based field representation includion-based analysis"),
        clEnumValN(PointerAnalysis::AndersenWaveDiff_WPA,
                   "wander",
        ...
        clEnumValN(PointerAnalysis::FSSPARSE_WPA,
                   "fspta",
                   "Sparse flow sensitive pointer analysis"),
        clEnumValN(PointerAnalysis::TypeCPP_WPA,
                   "type",
...
static llvm::cl::bits<WPAPass::AliasCheckRule> AliasRule(
    ...
    llvm::cl::values(clEnumValN(WPAPass::Conservative,
                                "conservative",
                                "return MayAlias if any pta says alias"),
                     clEnumValN(WPAPass::Veto,
                                "veto",
                                "return NoAlias if any pta says no alias")));

static llvm::cl::opt<bool> anderSVFG(
    "svfg",
    llvm::cl::init(false),
    llvm::cl::desc("Generate SVFG after Andersen's Analysis"));

static llvm::cl::opt<bool> printAliases(
    "print-aliases",
    llvm::cl::init(false),
...
void WPAPass::runOnModule(SVFModule svfModule) {
  runPointerAnalysis(svfModule, PointerAnalysis::Andersen_WPA);
  runPointerAnalysis(svfModule, PointerAnalysis::AndersenHCD_WPA);
  runPointerAnalysis(svfModule, PointerAnalysis::AndersenSCD_WPA);
  runPointerAnalysis(svfModule, PointerAnalysis::AndersenSFR_WPA);
  runPointerAnalysis(svfModule, PointerAnalysis::AndersenWaveDiff_WPA);
  runPointerAnalysis(svfModule, PointerAnalysis::AndersenLCD_WPA);
  runPointerAnalysis(svfModule, PointerAnalysis::AndersenHLCD_WPA);
...
void WPAPass::runPointerAnalysis(SVFModule svfModule, u32_t kind) {
  ...
  if (anderSVFG) {
    ...
    assert(SVFUtil::isa<Andersen>(_pta)
           && "supports only andersen for pre-computed SVFG");
...
void WPAPass::PrintAliasPairs(PointerAnalysis *pta) {
  PAG *pag = pta->getPAG();
  for (PAG::iterator lit = pag->begin(), elit = pag->end(); lit != elit;
       ++lit) {
...
ModRefInfo WPAPass::getModRefInfo(const CallInst *callInst) {
  ...
  return _svfg->getMSSA()->getMRGenerator()->getModRefInfo(
      SVFUtil::getLLVMCallSite(callInst));
}
...
ModRefInfo WPAPass::getModRefInfo(const CallInst *callInst1,
  ...
  assert(PASelected.isSet(PointerAnalysis::AndersenWaveDiff_WPA) && anderSVFG
         && "mod-ref query is only support with -ander and -svfg turned on");
  return _svfg->getMSSA()->getMRGenerator()->getModRefInfo(
      SVFUtil::getLLVMCallSite(callInst1),
      SVFUtil::getLLVMCallSite(callInst2));
}
