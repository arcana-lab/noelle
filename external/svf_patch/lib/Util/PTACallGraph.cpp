...
static llvm::cl::opt<bool> CallGraphDotGraph(
    "dump-callgraph",
    llvm::cl::init(false),
...
void PTACallGraph::buildCallGraph(SVFModule svfModule) {
  ...
  /// create nodes
  for (SVFModule::iterator F = svfModule.begin(), E = svfModule.end(); F != E;
       ++F) {
    addCallGraphNode(*F);
  }
...
void PTACallGraph::addDirectCallGraphEdge(const Instruction *call) {
  ...
  if (!hasGraphEdge(caller, callee, PTACallGraphEdge::CallRetEdge, csId)) {
    ...
    PTACallGraphEdge *edge = new PTACallGraphEdge(caller,
                                                  callee,
                                                  PTACallGraphEdge::CallRetEdge,
                                                  csId);
 ...
 */
void PTACallGraph::addIndirectCallGraphEdge(const Instruction *call,
                                            const Function *calleefun) {
  PTACallGraphNode *caller = getCallGraphNode(call->getParent()->getParent());
 ...
 */
bool PTACallGraph::isReachableBetweenFunctions(const Function *srcFn,
...
struct DOTGraphTraits<PTACallGraph *> : public DefaultDOTGraphTraits {
  ...
  static std::string getEdgeAttributes(PTACallGraphNode *node,
                                       EdgeIter EI,
                                       PTACallGraph *PTACallGraph) {
