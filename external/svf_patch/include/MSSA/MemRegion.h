...
public:
  ...
  inline std::string dumpStr() const {
    std::string str;
    str += "pts{";
    for (PointsTo::iterator ii = cptsSet.begin(), ie = cptsSet.end(); ii != ie;
         ii++) {
      char int2str[16];
...
public:
  ...
  typedef std::map<PointsTo, PointsTo, MemRegion::equalPointsTo>
      PtsToRepPtsSetMap;
...
private:
  ...
  /// Map a load PAG Edge to its memory regions sets in order for inserting mus
  /// in Memory SSA
...
protected:
  ...
  virtual inline void getMRsForLoad(MRSet &aliasMRs,
                                    const PointsTo &cpts,
                                    const Function *fun) {
  ...
  virtual bool handleCallsiteModRef(NodeBS &mod,
                                    NodeBS &ref,
                                    CallSite cs,
                                    const Function *fun);
  ...
  //@{
  inline void addCPtsToStore(PointsTo &cpts,
                             const StorePE *st,
                             const Function *fun) {
  ...
  inline void addCPtsToLoad(PointsTo &cpts,
                            const LoadPE *ld,
                            const Function *fun) {
...
public:
  ...
  inline ModRefInfo getModRefInfo(const CallInst *call,
                                  const CallInst *otherCall) {
    return getModRefInfo(SVFUtil::getLLVMCallSite(call),
                         SVFUtil::getLLVMCallSite(otherCall));
  }
