...
public:
  /// Constructor
  PTACallGraphEdge(PTACallGraphNode *s,
                   PTACallGraphNode *d,
                   CEDGEK kind,
                   CallSiteID cs)
  ...
  void addDirectCallSite(const Instruction *call) {
    assert((SVFUtil::isa<CallInst>(call) || SVFUtil::isa<InvokeInst>(call))
           && "not a call or inovke??");
  ...
  void addInDirectCallSite(const Instruction *call) {
    assert((SVFUtil::isa<CallInst>(call) || SVFUtil::isa<InvokeInst>(call))
           && "not a call or inovke??");
    assert((NULL == SVFUtil::getCallee(call)
            || NULL == SVFUtil::dyn_cast<Function>(SVFUtil::getForkedFun(call)))
...
public:
  /// Constructor
  PTACallGraphNode(NodeID i, const Function *f)
    : GenericCallGraphNodeTy(i, 0),
      fun(f) {}
...
public:
  typedef PTACallGraphEdge::CallGraphEdgeSet CallGraphEdgeSet;
  typedef llvm::DenseMap<const Function *, PTACallGraphNode *>
      FunToCallGraphNodeMap;
  typedef llvm::DenseMap<const Instruction *, CallGraphEdgeSet>
...
public:
  ...
  inline const CallInstToCallGraphEdgesMap &getCallInstToCallGraphEdgesMap()
      const {
  ...
  PTACallGraphEdge *hasGraphEdge(PTACallGraphNode *src,
                                 PTACallGraphNode *dst,
                                 PTACallGraphEdge::CEDGEK kind,
                                 CallSiteID csId) const;
  /// Get call graph edge via nodes
  PTACallGraphEdge *getGraphEdge(PTACallGraphNode *src,
                                 PTACallGraphNode *dst,
                                 PTACallGraphEdge::CEDGEK kind,
                                 CallSiteID csId);
  ...
  inline CallGraphEdgeSet::const_iterator getCallEdgeBegin(
  ...
  inline CallGraphEdgeSet::const_iterator getCallEdgeEnd(
      const Instruction *inst) const {
    CallInstToCallGraphEdgesMap::const_iterator it =
        callinstToCallGraphEdgesMap.find(inst);
    assert(it != callinstToCallGraphEdgesMap.end()
  ...
  void addIndirectCallGraphEdge(const Instruction *call,
                                const Function *callee);
  //@}

  /// Get callsites invoking the callee
  //@{
  void getAllCallSitesInvokingCallee(const Function *callee,
                                     PTACallGraphEdge::CallInstSet &csSet);
  void getDirCallSitesInvokingCallee(const Function *callee,
                                     PTACallGraphEdge::CallInstSet &csSet);
  void getIndCallSitesInvokingCallee(const Function *callee,
                                     PTACallGraphEdge::CallInstSet &csSet);
  //@}

  /// Whether its reachable between two functions
  bool isReachableBetweenFunctions(const Function *srcFn,
                                   const Function *dstFn) const;
 ...
 */
template <>
struct GraphTraits<PTACallGraphNode *>
  : public GraphTraits<GenericNode<PTACallGraphNode, PTACallGraphEdge> *> {};

/// Inverse GraphTraits specializations for call graph node, it is used for
/// inverse traversal.
template <>
struct GraphTraits<Inverse<PTACallGraphNode *>>
  : public GraphTraits<
        Inverse<GenericNode<PTACallGraphNode, PTACallGraphEdge> *>> {};

template <>
struct GraphTraits<PTACallGraph *>
  : public GraphTraits<GenericGraph<PTACallGraphNode, PTACallGraphEdge> *> {
