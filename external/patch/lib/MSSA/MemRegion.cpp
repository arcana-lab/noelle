...
void MRGenerator::collectModRefForLoadStore() {
  ...
  for (SVFModule::iterator fi = svfModule.begin(), efi = svfModule.end();
       fi != efi;
       ++fi) {
    ...
    for (Function::const_iterator iter = fun.begin(), eiter = fun.end();
      ...
      for (BasicBlock::const_iterator bit = bb.begin(), ebit = bb.end();
        ...
        for (PAGEdgeList::iterator bit = pagEdgeList.begin(),
...
void MRGenerator::collectModRefForCall() {
  ...
  for (PAG::CallSiteSet::const_iterator
 ...
 */
void MRGenerator::addRefSideEffectOfFunction(const Function *fun,
                                             const NodeBS &refs) {
 ...
 */
void MRGenerator::addModSideEffectOfFunction(const Function *fun,
                                             const NodeBS &mods) {
...
void MRGenerator::collectCallSitePts(CallSite cs) {
  ...
  if (pta->getPAG()->hasCallSiteArgsMap(cs)) {
    ...
    for (PAG::PAGNodeList::const_iterator itA = args.begin(), ieA = args.end();
         itA != ieA;
         ++itA) {
