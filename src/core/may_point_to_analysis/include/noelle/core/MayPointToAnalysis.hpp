...
class Pointer {
public:
  Pointer(Value *source) : source(source){};
  Value *source;
...
public:
  Variable(Value *source) : Pointer(source){};
...
public:
  MemoryObject(Value *source, bool prev = false) : Pointer(source) {
    prevLoopAllocated = prev;
  };
  PointNodeType getType() override {
...
using PointToInfo = std::unordered_map<Pointer *, MemoryObjects>;
using StoreRecord = PointToInfo;

class FunctionSummary {
public:
  FunctionSummary(Function *F);
  std::unordered_set<CallInst *> heapAllocInsts();
  std::unordered_set<AllocaInst *> stackAllocInsts();
...
public:
  ...
  MemoryObject *mustPointToMemory(Instruction *inst, Pointer *pointer);
  MemoryObjects pointees(PointToInfo &ptInfo, Pointer *ptr);
  MemoryObjects pointees(PointToInfo &ptInfo, Value *ptr);
  MemoryObjects reachableMemoryObjects(PointToInfo &ptInfo, Pointer *ptr);
  Variable *getVariable(Value *value);
  MemoryObject *getMemoryObject(Value *value);

  Module *M;
  std::unordered_map<Instruction *, PointToInfo> instIN;
  std::unordered_map<Instruction *, PointToInfo> instOUT;
  std::unordered_map<BasicBlock *, PointToInfo> bbIN;
  std::unordered_map<BasicBlock *, PointToInfo> bbOUT;
...
private:
  ...
  PointToInfo mergeAllPredOut(BasicBlock *bb, PointToSummary *ptSum);
  PointToInfo FS(Instruction *inst, PointToSummary *ptSum);
};
