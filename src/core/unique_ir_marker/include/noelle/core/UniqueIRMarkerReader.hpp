#ifndef NOELLE_SRC_CORE_UNIQUEIRMARKERREADER_H
#define NOELLE_SRC_CORE_UNIQUEIRMARKERREADER_H
#pragma once
...
private:
  ...
  static optional<reference_wrapper<const MDOperand>> getIthOperand(
      const MDNode *,
      uint operand);

  template <typename T>
  static optional<IDType> getIDConst(
      const T *,
      std::function<optional<Constant *>(const T *)>);
