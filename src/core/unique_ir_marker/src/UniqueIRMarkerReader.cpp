...
optional<Constant *> UniqueIRMarkerReader::getModuleConstID(
    const llvm::Module *M) {
  assert(M && "Not a valid Module");
  auto *metaNode = M->getNamedMetadata(Twine(UniqueIRConstants::VIAModule));
  assert(
      metaNode
      && "No ID for module it is likely that the unique IR identifier pass (adding metadata with IDs) has "
         "not been run");
  assert(metaNode->getNumOperands() == 1
         && "Should only have one meta node operand");
  assert(metaNode->getNumOperands() == 1
         && metaNode->getOperand(0)->getNumOperands() == 1
...
optional<Constant *> UniqueIRMarkerReader::getLoopConstID(const llvm::Loop *L) {
  ...
  if (!possibleVIAStrID
      || !possibleVIAStrID->getString().equals(UniqueIRConstants::VIALoop)) {
    return nullopt;
  }

  auto possibleVIAID =
      dyn_cast<ConstantAsMetadata>(possibleVIAIDPair->getOperand(1));
  if (!possibleVIAID) {
    return nullopt;
  }
...
optional<Constant *> UniqueIRMarkerReader::getBasicBlockConstID(
...
optional<Constant *> UniqueIRMarkerReader::getFunctionConstID(
...
optional<IDType> UniqueIRMarkerReader::getBasicBlockID(
    const llvm::BasicBlock *BB) {
  return getIDConst<BasicBlock>(BB, getBasicBlockConstID);
}

optional<IDType> UniqueIRMarkerReader::getInstructionID(
    const llvm::Instruction *I) {
...
optional<Constant *> UniqueIRMarkerReader::getConst(
    const llvm::MDOperand &node) {
  auto *ConstMeta = dyn_cast<ConstantAsMetadata>(node);
  return ConstMeta ? optional<Constant *>(ConstMeta->getValue()) : nullopt;
}

optional<reference_wrapper<const MDOperand>> UniqueIRMarkerReader::
    getIthOperand(const MDNode *node, uint operand) {
  if (node && node->getNumOperands() == (operand + 1)) {
    return optional<reference_wrapper<const MDOperand>>{ node->getOperand(
        operand) };
  }
  return std::nullopt;
}
...
template <typename T>
optional<IDType> UniqueIRMarkerReader::getIDConst(
    const T *t,
    std::function<optional<Constant *>(const T *)> f) {
  auto *x = f(t).value_or(nullptr);
  return x ? getID(x) : nullopt;
}

optional<Constant *> UniqueIRMarkerReader::getConstFromMeta(llvm::MDNode *node,
                                                            uint operand) {
  if (!node)
    return nullopt;
