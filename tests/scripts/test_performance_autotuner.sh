...
function runningTests {
  ...
  for i in `ls`; do
    ...
    # echo "   Make " ;
    make baseline ;
    make parallelized_autotuned TIME="600" SEED_CONF="autotuner_seed.info" NOSEARCHSPACE="--nosearchspace" NODOALLFILTERING="--noDOALLfiltering" NODOALLTIMEFILTERING="--noDOALLtimefiltering" INPUT="perf_args.info" NOELLE_OPTIONS="$2" PARALLELIZATION_OPTIONS="$parOptions" RUNTIME_CFLAGS="-DNDEBUG" >> compiler_output.txt 2>&1 ;
    ...
    measureTime parallelized_autotuned time_parallelized.txt output_parallelized.txt ;
